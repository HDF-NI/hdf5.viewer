...
app.use((ctx, next) => {
    if (ctx.originalUrl === "/api/test-tree") {
        ctx.type = 'application/json';
        try {
            const theTree = new H5Tree(global.currentH5Path);
            
            // Execute the load call out-of-band without stream piping
            // Note: Since this app uses generator routing context via yield/co, 
            // we wrap it in a direct promise handler to output the raw layout.
            const co = require('co');
            return co(function* () {
                var h5Metadata = yield theTree.loadH5Metadata();
                ctx.body = JSON.stringify(h5Metadata);
            }).catch(err => {
                ctx.status = 500;
